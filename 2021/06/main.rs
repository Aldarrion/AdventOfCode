...
fn main() {
    ...
    let fish_input: Vec<i32> = line_iter.next().unwrap().unwrap().split(',').map(|n| n.parse::<i32>().unwrap()).collect();

    let mut fish: [i64;9] = Default::default();
    for f_i in fish_input.iter() {
        fish[(*f_i) as usize] += 1;
    }

    for _ in 0..256 {
        let mut new_fish: [i64;9] = Default::default();
        new_fish[8] = fish[0];
