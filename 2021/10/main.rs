...
fn main() {
    ...
    let opening = ['(', '[', '{', '<'];
    let matching = [')', ']', '}', '>'];
    let score_mapping = [3, 57, 1197, 25137];

    let mut corrupted_score = 0;
    let mut incomplete_scores = Vec::<i64>::new();
        ...
        if let Ok(l) = line {
            ...
            for c in l.chars() {
                if opening.contains(&c) {
                    let pos = opening.iter().position(|x| *x == c).unwrap();
                    stack.push(pos as i8);
                } else {
                    let open = stack.pop().unwrap() as usize;
                    if c != matching[open] {
                        let pos = matching.iter().position(|x| *x == c).unwrap();
                        corrupted_score += score_mapping[pos];
                        corrupted = true;
                        break;
                    }
                }
